function goal_weight() {
    ...
    if(gw>29 && gw<201)
    {
        ...
        $("input[name=item3]").attr("value",tgw);
...
function go_sub(){
    ...
    $('.quest00').attr("onclick","jump_quest(0)");
...
function now_weight() {
    ...
    if(total>0)
    {
        ...
        $("input[name=item1]").attr("value",tnw);
        $('#next_nw').css({transition:"1s"});
  ...
  function maxLengthCheck(object){										
   ...
   function now_phone() {
    ...
    if(ph.substr(0, 3) == "010" && ph.length==11)
    {
        ...
        $("input[name=tel1]").val(ph.substr(0,3));
...
function purchase(){
    fbq('track', 'Purchase', {currency: "KRW", value: 25000});
}
